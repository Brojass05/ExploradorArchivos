...
# Módulos de PyQt5 para la interfaz gráfica
...
from PyQt5.QtCore import Qt, QModelIndex, QRegExp, QFileInfo
...
# Módulos específicos de Windows (si aplican y son externos a la stdlib)
import ctypes # Para interactuar con APIs de C (e.g., verificar permisos de administrador).
...
# (Funciones que no pertenecen a ninguna clase específica pero son usadas por varias)
# ==============================================================================
...
# (Ventanas emergentes para la interacción con el usuario)
# ==============================================================================
...
class FileExplorer(QMainWindow):
    ...
    def __init__(self):
        ...
        self.boton_formateo.clicked.connect(self._handle_formateo_boton) # Usar un manejador específico
        ...
        self.boton_particion.clicked.connect(self._handle_particion_boton) # Usar un manejador específico
        ...
        self.boton_carpeta.clicked.connect(self._handle_carpeta_boton) # Usar un manejador específico
        self.toolbar.addWidget(self.boton_carpeta)

        # Habilitar el menú contextual personalizado en el árbol (clic derecho).
        self.tree.setContextMenuPolicy(Qt.CustomContextMenu)
        self.tree.customContextMenuRequested.connect(self._on_tree_right_click) # Conecta el evento de clic derecho a un método.
    ...
    def _handle_carpeta_boton(self):
        """
        ...
        try:
            ...
            if not index.isValid():
                ...
                return
            self._crear_carpeta(index) # Llama al método para crear una carpeta en la unidad/carpeta seleccionada.

        except Exception as e:
            QMessageBox.warning(self, "Error", f"No se pudo iniciar la creación de carpeta. Error: {e}")
            return

    def _handle_formateo_boton(self):
        """
        Maneja la acción de formatear una unidad desde el botón de la barra de herramientas.
        ...
        if seleccion == True: # Si el usuario confirmó la selección en el diálogo.
            ...
            if reply == QMessageBox.Yes: # Si el usuario confirma.
                self._formatear_disco(disco, sistema_archivos, etiqueta) # Llama al método para formatear.

    def _handle_particion_boton(self):
        """
        Maneja la acción de particionar una unidad desde el botón de la barra de herramientas.
        Muestra un diálogo para seleccionar la unidad y luego llama a _auxiliar_particion.
        """
        unidades = obtener_unidades()
        ...
        if ok and letra:
            self._auxiliar_particion(letra) # Llama al método auxiliar para manejar la partición de la unidad seleccionada.
                    ...
                    case "Formatear":
                        if Letra != "C:": # No permite formatear la unidad C: por seguridad del sistema operativo.
                            ...
                            if seleccion == True: # Si el usuario confirmó la selección en el diálogo.
                                ...
                                if reply == QMessageBox.Yes: # Si el usuario confirma.
                                    self._formatear_disco(Letra, sistema_archivos, etiqueta) # Llama al método para formatear.
    ...
    def click_en_arbol(self, index: QModelIndex):
        ...
        archivos = self._obtener_archivos_de_ruta(ruta) # Usa el método auxiliar privado
    ...
    # ==================== Métodos de Gestión de Archivos y Carpetas ====================

    def _crear_carpeta(self, index: QModelIndex):
        """
        ...
        """
        name = self._get_nombre_carpeta() # Pide al usuario un nombre para la nueva carpeta.
    ...
    def _get_nombre_carpeta(self):
        ...
        if ok and text:
            ...
            # El mensaje de error se maneja en el método que llama a esta función (`_crear_carpeta`).
            return None

    def _obtener_archivos_de_ruta(self, ruta):
        """
    ...
    # ==================== Métodos de Gestión de Discos ====================

    def _formatear_disco(self, letra, sistema_archivo, nuevo_nombre):
        """
    ...
    def _auxiliar_particion(self, letra):
        """
        ...
        y la letra, y luego llama a _crear_particion.

        Args:
            letra (str): La letra de la unidad a particionar (ej. "D:").
        """
        info = self._obtener_disco_fisico(letra)
        ...
        if tamano_particion is not None and nueva_letra is not None:
            ...
            if reply == QMessageBox.Yes:
                ...
                self._crear_particion(info['Number'], letra, tamano_particion, nueva_letra, nombre_particion)

    def _obtener_disco_fisico(self, letra_unidad):
        """
    ...
    def _crear_particion(self, disco_id, letra_original, tamaño_MB, nueva_letra, nombre_etiqueta):
        """
        ...
        # resShrink.stdout.close() # No es necesario si ya se usó .wait() y no se va a leer más.
        ...
        # resParticion.stdout.close() # No es necesario
    ...
    def _obtener_informacion_particion(self, letra_unidad):
        """
        ...
        que usa '_obtener_disco_fisico' para el espacio libre).
